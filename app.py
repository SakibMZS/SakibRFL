...
# ============================================
def setup_view():
    """Renders the file upload landing page with NO sidebar."""
...
def dashboard_view():
    """Renders the active dashboard console with pinned sidebar."""
    all_parsed_dfs = []
    all_audit_dfs = []
    ...
    if floor_choice == "FF" and "ff_bytes" not in st.session_state:
        ...
        st.warning(
            "⚠️ **Ground Floor (GF) file is not uploaded.** Please click '⚙️"
            " Change Uploaded Files' in the sidebar to upload the GF file."
        )
        return
    ...
    # ============================================
        ...
        if mtd_mode == "📊 Linewise":
            ...
            for (cust, ord_name, acc_cd), grp in df_mtd.groupby(
                ...
                as_of_pct = (
                    (as_of_prod / order_qty * 100) if order_qty > 0 else 0.0
                )
            ...
            if df_job_mtd.empty:
                ...
                with st_tab1:
                    ...
                    st.dataframe(
                        clean_and_format_dataframe(df_active_tot[v_cols]),
                        use_container_width=True,
                        hide_index=True,
                    )
                ...
                with st_tab2:
                    ...
                    if df_done_jobs.empty:
                        ...
                        st.dataframe(
                            clean_and_format_dataframe(df_done_tot[v_cols]),
                            use_container_width=True,
                            hide_index=True,
                        )
